...
describe("redirectWithOutcome function utility", () => {
  it("Should redirect to the correct url and correct outcome", () => {
    ...
    redirectWithOutcome(OUTCOME_ROUTE.INVALID_DATA);
    expect(global.location.href).toContain("outcome=3");
    redirectWithOutcome(OUTCOME_ROUTE.TIMEOUT);
    expect(global.location.href).toContain("outcome=4");
    redirectWithOutcome(OUTCOME_ROUTE.INVALID_CARD);
    expect(global.location.href).toContain("outcome=7");
    redirectWithOutcome(OUTCOME_ROUTE.CANCELED_BY_USER);
    expect(global.location.href).toContain("outcome=8");
    redirectWithOutcome(OUTCOME_ROUTE.INVALID_SESSION);
    expect(global.location.href).toContain("outcome=14");
    redirectWithOutcome(OUTCOME_ROUTE.ALREADY_ONBOARDED);
    expect(global.location.href).toContain("outcome=15");
    redirectWithOutcome(OUTCOME_ROUTE.PSP_ERROR);
    expect(global.location.href).toContain("outcome=25");
    redirectWithOutcome(OUTCOME_ROUTE.BALANCE_LIMIT);
    expect(global.location.href).toContain("outcome=116");
    redirectWithOutcome(OUTCOME_ROUTE.CVV_ERROR);
    expect(global.location.href).toContain("outcome=117");
    redirectWithOutcome(OUTCOME_ROUTE.LIMIT_EXCEEDED);
    expect(global.location.href).toContain("outcome=121");
