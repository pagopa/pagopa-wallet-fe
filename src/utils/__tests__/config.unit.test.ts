...
describe("Env config initialization", () => {
  it("Should return all configs value correctly", () => {
    let config = getValidConfig();
    expect(config.WALLET_CONFIG_API_BASEPATH).toEqual("/webview-payment-wallet/v1");
    ...
    expect(config.WALLET_CONFIG_WEBVIEW_PM_HOST).toEqual("https://api.dev.platform.pagopa.it");
    expect(config.WALLET_GDI_CHECK_TIMEOUT).toEqual(30000);
    expect(config.WALLET_NPG_SDK_URL).toEqual("https://stg-ta.nexigroup.com/monetaweb/resources/hfsdk.js");
    expect(config.WALLET_OUTCOME_API_BASEPATH).toEqual("/payment-wallet");
    expect(config.WALLET_PAGOPA_LOGOS_CDN).toEqual("https://assets.cdn.io.italia.it/logos/abi");
